...
fn setup_game_config(game_name: &str, game_dir: &str, mod_dir: &str) -> Result<(), String> {
    log::info!(
...
#[tauri::command(rename_all = "snake_case")]
fn read_game_config(game_name: &str) -> Result<serde_json::Value, String> {
    ...
    Ok(serde_json::to_value(game_config.value()).unwrap())
}
...
mod tests {
    ...
    fn test_save_config() {
        env_logger::builder().filter_level(LevelFilter::Info).init();
        let _ = setup_game_config("test", ".", "src");
        save_game_config();
    }
